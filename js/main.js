var checkColor = function() {
  ...
  for (var i = 0; i < 100; i++) {
    var p = context.getImageData(i, i, 1, 1).data;
    var hex = "#" + ("000000" + rgbToHex(p[0], p[1], p[2])).slice(-6);
    if (colors.includes(hex) && colors.length > 1) {
      return true;
    }
...
var isDescending = function() {
  var arr = [];
  $(".songlist span").each(function(index, elem) {
    var wordCount = $(this).text().split(":");
    arr.push(parseInt(wordCount[1].trim()));
  });
...
var clickAWord = function() {
  // var cX = $('#canvas-container').offset().left + $('#canvas-container').width() / 2;
  // var cY = $('#canvas-container').offset().top + $('#canvas-container').height() / 2;
...
//Will contain all the lyrics as one long string, space delimited
var totalLyrics = "";
